...
- **normal preferences**: Removed AeonSettingsCard and bespoke Apply/confirmation/readback-wait UI. Standard Display controls for wheel brightness/units and Speech control for wheel button sounds use generic WheelPreferenceChange, mapped in NosfetAeonProtocol. No model branches in widgets. Connection, range, model and known-support checks remain; former stationary/charging/freshness/12-second wait gates removed for these preferences at owner request. Raw Inspector, headlight and speed readbacks remain.
- **acknowledgement policy**: Fresh page8 SND>0 selects original paired headlight path; SND0 or unavailable/stale selects silent ASCII. Transaction snapshot prevents mismatched primary/companion after SND changes. Settings share selector but have no known silent variant, so retain existing beeping packets. Horn remains intentional sound; speed/alarm command policy unchanged. Not a universal mute bit or volume scaling. New automatic selection requires wheel validation.
- **legal mode restore**: LegalModeSpeedMemory stores the configured normal pair before applying Legal mode, keyed by BLE/virtual-wheel address. Both telemetry and Settings adoption paths preserve normal values while the snapshot is protected. Restore uses the original pair, not overwritten readbacks. Repeated enable and same-process reconnect retain the snapshot. It is released only after a later matching pair; missing or partial feedback retains it. Memory is not persistent across app process death. No changes to command bytes, transport failure handling or optimistic Legal UI; no claim of successful wheel execution.
